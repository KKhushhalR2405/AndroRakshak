...
def home():
    ...
    result, accuracy, name, sdk, size ,perm,timetime = '', '', '', '', '','',''
    if request.method == "POST":
        ...
        if file and file.filename.endswith('.apk'):
            ...
            if request.form['algorithm'] == 'Neural Network':
                ...
                result, name, sdk, size, perm ,timetime= classifier.classify(os.path.join(app.config['UPLOAD_FOLDER'], filename), 0)
            elif request.form['algorithm'] == 'Support Vector Classifier':
                accuracy = algorithms['Support Vector Classifier']
                result, name, sdk, size, perm,timetime = classifier.classify(os.path.join(app.config['UPLOAD_FOLDER'], filename), 1)
    return render_template("index.html", result=result, algorithms=algorithms.keys(), accuracy=accuracy, name=name,
                           sdk=sdk, size=size,perm = perm,timetime=timetime)
